...
def process_file(bucket: str, key: str):
    ...
    try:
        ...
        logger.info(f"Processing started for file: s3://{bucket}/{key}")
        ...
        for col in NUMERIC_COLS:
            if col in df.columns:
                ...
                if clean_values:
                    ...
                    logger.info(
                        f"Baseline updated for {col}, "
                        f"new count={baseline[col]['count']}"
                    )
        ...
        logger.info(f"Processed file saved: {output_key}")
        ...
        logger.info(
            f"Processing complete: {anomaly_count}/{len(df)} anomalies detected"
        )
        ...
        logger.error(f"Processing failed for {key}: {e}", exc_info=True)
        print(e)
